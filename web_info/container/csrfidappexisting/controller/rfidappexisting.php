...
class Rfidappexisting extends Controller{
	...
	function init(){
		...
		$this->view->render("templateadmin","abr/rfidexisting_view");
	...
	function createretailer(){
		...
		try{
			...
			if(count($rindt)==0){
				echo json_encode(array('message'=>'RIN Not found!','result'=>'error'));
				exit;
			}
			...
			$st = "update mlminfo set xcreditterms=2, xarc='".$arccode."', bloodg='".$bloodg."' where xrdin='".$rin."'";
	...
	function script(){
		...
		$(document).ready(function(e){
			...
			$('#retailerform').on('submit',(function(e) {
				...
				$.ajax({
					type:'POST',
					url:\"".URL."rfidappexisting/createretailer\", 
					data:formData,
					cache:false,
					contentType: false,
					processData: false,
			...
			$('#rin').on('blur', function(){
				...
				$.ajax({
					...
					url:\"".URL."rfidappexisting/showretailer\", 
					type : \"POST\",                                  				
