...
class Bcupgrade_Model extends Model{
	...
	function bininfo($bin=0){
		return $this->db->select('mlmtree',array("binpoint,(select centerpoint from mlmtree_matching where bin=$bin) as centerpoint,binstatus"), " distrisl=".Session::get('sdistrisl')." and bin=$bin");
	}
