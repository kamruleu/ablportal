...
class Myacc extends Controller{
    ...
    function otherledger(){
        $this->view->render("templateadmin","abr/otherledger_view");
    }
    ...
    function showotherledger(){
        ...
        //Logdebug::appendlog($stfrom.' '.$stto.' '.$searchstr);
        $this->model->myotherledger($stfrom, $stto);
    }
    ...
    function script(){
        return "
            <script>
                ...
                $('#searchtxnother').on('click',function(){
                    $.ajax({
                        
                        url:\"".URL."comledger/showotherledger\", 
                        type : \"POST\",
                        dataType: 'json',                                  				
                        data : {stfrom:$('#stfromother').val(),stto:$('#sttoother').val()}, // post data || get data                    
                        beforeSend:function(){	
                            loaderon();   
                            $('#txntableother tbody').html('');
                            $('#totalamtother').html('0')
                        },
                        success : function(result) {
                           //console.log(result)
                            loaderoff();
                            ...
                            $.each(result, function(key,val){
                                totalmt+=parseFloat(val['com']);
                                $('#txntableother tbody').append(`<tr><td>`+val['txndate']+`</td><td>`+val['stno']+`</td><td>`+val['comtype']+`</td><td>`+val['com']+`</td></tr>`);
                            })
                            $('#totalamtother').html(totalmt.toFixed(2))
                        }
