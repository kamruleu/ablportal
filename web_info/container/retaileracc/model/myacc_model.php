...
class Myacc_model extends Model{
    ...
    function myotherledger($stfrom,$stto){
        $where  = " stno between $stfrom and $stto and xrdin = '".Session::get('suser')."'  order by stno desc";
        
        
        //$this->log->modellog($where);
        echo json_encode($this->db->select('expresstotrep',array("DATE_FORMAT(xdate,'%d-%m-%Y') as txndate,stno,xcomtype as comtype,xcom as com"),$where));
    }
