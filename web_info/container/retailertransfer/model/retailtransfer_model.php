...
class Retailtransfer_model extends Model{
    ...
    function txnsearchall(){
        ...
        return $this->db->select('imretailtransfer m, mlminfo c',array("DATE_FORMAT(m.xdate,'%d-%m-%Y') as txndate,m.xtransnum as txnnum,m.xrdin as cin,c.xorg as cusname, c.xmobile as mobile,(select COALESCE(sum(xqty*xbv),0) from imretaildet d where xdoctype='Retailer Stock Transfer' and m.xtransnum=d.xtxnnum) as totbv"),$where);

    }
    
    function txnrcvsearch($searchcol,$searchval){

        $where = " m.zemail = c.xrdin and ".$searchcol." like '%".$searchval."%' and m.xrdin = '".Session::get('suser')."'";
    ...
    function txnrcvsearchall(){
        ...
        $where = " m.zemail = c.xrdin and m.xrdin = '".Session::get('suser')."' order by m.ztime desc";
