...
class Nagadpay extends Controller{
    ...
    function paybynagad($token){
        ...
        foreach($data->itemdt as $key=>$value){
            ...
            if(count($itemdt)>0){
                ...
                $vals .= "('".$invoice_no."','".$data->customertype."','".$data->customer."','".$data->cusname."',$stno,'Nagad','".$data->add1."','".$data->city."','".$data->district."','".$data->mobile."','".$cusdt[0]['xpostal']."','".$value->itemcode."','".$value->qty."','".$itemdt[0]['xstdprice']."','".$itemdt[0]['xdp']."','".$data->callbackurl."','".$data->doctype."','".$data->odcid."','".$data->company."','".$data->outlet."','".$data->doc."','".$data->refrin."'),";
            }
        }
        ...
        $merchantCallbackURL = "https://portal.amarbazarltd.com/nagadpay/callback";
    ...
    function callback(){
        ...
        if($tord[0]['xstatus']=="Success"){
            $this->finalcallback("", "Failed", $hostcallback);
        }
        $isordered = $this->model->isordered($arr['orderId']);
...
function HttpPostMethod($PostURL, $PostData)
{
    ...
    $ResultArray = json_decode($resultdata, true); //Logdebug::appendlog(print_r($resultdata, true));
    curl_close($url);
    return $ResultArray;
