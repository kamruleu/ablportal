...
class Cusactivate extends Controller{
	...
	function activatecustomer(){
		...
		try{
			...
			$cusdt = $this->model->getcindt(filter_var($_POST["cin"], FILTER_SANITIZE_STRING));
			
			if(count($cusdt)==0){
			    throw new Exception(serialize(array('result'=>'error','message'=>'CIN not found!')));
					exit;
			}
			...
			if( (int)$success>0){
				...
				$sendsms = new Sendsms();
				$sendsms->send_single_sms('আপনার কাস্টমার নম্বর '.$data["xcus"].'এবং আপনার কার্ড নম্বর '.$data['xid'], $cusdt[0]['mobile']);
