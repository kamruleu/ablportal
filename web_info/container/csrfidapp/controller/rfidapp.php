...
class Rfidapp extends Controller{
	...
	function init(){
		...
		$this->view->render("templateadmin","abr/rfid_view");
	...
	function createretailer(){
		...
		$arccode = filter_var($_POST['arccode'], FILTER_SANITIZE_STRING);
		...
		try{
			...
			$isused = $this->model->iscinused($gettxn[0]['xrdin']);
			if(count($isused)>0){
			...
			$cusdt = $this->model->getcusdt($gettxn[0]['xrdin']);	
			
			$refrindt = $this->model->getrindt($_POST['refrin']);
			...
			$data['cbin']='0';
			$data['xcreditterms']='2';//for id status like 2 is applied, 3 processing, 4 pending for delivery
			$data['xpassword']=$password;
			$data['zemail']=Session::get('suser'); //Logdebug::appendlog(serialize($data));
			
			$success = $this->model->createretailer('mlminfo',$data);
			//echo $success;	
			if($success>0){
				...
				$adata = array(
					...
					'point' => 25,
					'xdocnum'=>$txnno,
					'xdoctype'=>'EXPRESS Card Payment',
					'xsign'=>1,
					'xdate'=>$date,
					'stno'=>$stno,
					'zemail'=>Session::get('suser')
				);
				$success = $this->model->createretailer('mlmbv',$adata);
				if ($_FILES["photofile"]["name"]){
					$imgupload = new ImageUpload();
					$imgupload->store_uploaded_image('public/images/bizimages/retailer/','photofile', 300, 300, $rin."-1");
	...
	function loadarc(){
        ...
        try{
            ...
            echo json_encode(array('message'=>$this->model->getarc(),'result'=>'success'));
        }catch(Exception $e){
            $res = unserialize($e->getMessage()); 
	...
	function script(){
		...
		function loadarc(){
			$.ajax({
				...
				url:\"".URL."rfidapp/loadarc\", 
				type : \"POST\", 
				//dataType: \"json\",                                 				
				data : {token: $('#token').val()}, // post data || get data                    
				beforeSend:function(){	
					$('#arccode').html('<option default>Select ARC</option>')
					loaderon();   
				},
				success : function(result) {
					...
					if(resultobj['result']=='success'){
						$.each(resultobj['message'], function(key, val){
							$('#arccode').append('<option value=\"'+val['arc']+'\">'+val['arc']+'</option>');
						})
					}
					loaderoff();
		...
		$(document).ready(function(e){
			...
			$('#retailerform').on('submit',(function(e) {
				...
				$.ajax({
					type:'POST',
					url:\"".URL."rfidapp/createretailer\", 
					data:formData,
					cache:false,
					contentType: false,
					processData: false,
			...
			$('#searchcus').on('click', function(){
				...
				$.ajax({
					...
					url:\"".URL."rfidapp/searchcustomer\", 
					type : \"POST\",                                  				
