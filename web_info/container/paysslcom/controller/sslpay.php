    ...
    function makepayment($token){
        ...
        foreach($data->itemdt as $key=>$value){
            ...
            if(count($itemdt)>0){
                ...
                $numitems+=1;
                $vals .= "('".$invoice_no."','".$data->customertype."','".$data->customer."','".$data->cusname."',$stno,'SSLCOMMERZE','".$data->add1."','".$data->city."','".$data->district."','".$data->mobile."','".$cusdt[0]['xpostal']."','".$value->itemcode."','".$value->qty."','".$itemdt[0]['xstdprice']."','".$itemdt[0]['xdp']."','".$data->callbackurl."','".$data->doctype."','".$data->odcid."','".$data->company."','".$data->outlet."','".$data->doc."','".$data->refrin."'),";
            }
        }
        $vals = rtrim($vals,",");
