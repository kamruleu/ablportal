...
class Pointofsale_model extends Model{
    ...
    function getinvoicedetail($invoice){
        ...
        $salesheader = $this->db->dbselectbyparam('imretailsale m',"xsalesnum as txnnum,DATE_FORMAT(xdate,'%d-%m-%Y') as entrydate,xcus as cin,(select xorg from secus c where m.xcus=c.xcus) as cusname,(select xorg from secus c where m.xcus=c.xcus) as cusname,(select xmobile from secus c where m.xcus=c.xcus) as cusmobile,(select xadd1 from secus c where m.xcus=c.xcus) as cusadd, xstatus as status",$conditions,$params);
    ...
    function getitemdt($item){
        ...
        $params[]= $item;
        $params[]= 1;   
		return $this->db->dbselectbyparam('seitem','xitemcode,xdesc,xdp,xmrp as xstdprice,xstdcost',$conditions,$params);
