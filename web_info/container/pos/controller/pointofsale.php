                  ...
                  $('#searchtxn').on('click', function(){
                    ...
                    $.ajax({
                        ...
                        success : function(result) {
                            ...
                            if(resultobj['result']=='success'){
                                $.each(resultobj['message'], function(key, val){
                                    $('#txntable tbody').append(`<tr><td>`+val['txndate']+`</td><td>`+val['txnnum']+`</td><td>`+val['cin']+`</td><td>`+val['cusname']+`</td><td>`+val['mobile']+`</td><td><a href=\"javascript:void(0)\" onclick=\"showinvoiceitem(\``+val['txnnum']+`\`)\">View Detail</a></td><td><a target=\"_blank\" href=\"".URL."pos/printinvoice/`+val['txnnum']+`\">Print</a></td></tr>`);
