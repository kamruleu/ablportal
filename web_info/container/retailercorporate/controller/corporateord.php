...
class Corporateord extends Controller{
    ...
    function init(){
        ...
        $this->view->render("templateadmin","abr/corporateord_view");
    }
    ...
    function getsuplist(){
        echo json_encode($this->model->supplierlist());
    }
    ...
    function searchtxn(){
        ...
        try{
            ...
            if($col=='by Invoice')
                ...
                $where = " m.xdoctype='Corporate Order'";
            
                if($text!="" && !$isdaterange)
                    $where .= "  and m.xtxnnum=c.imretailsl and ".$col." like '%".$text."%' and c.zemail = '".Session::get('suser')."'";
            
                if($text=="" && $isdaterange)
                    $where .= " and m.xtxnnum=c.imretailsl and m.xdate between '".$startdate."' and '".$enddate."' and c.zemail = '".Session::get('suser')."'";

                
                if($text!="" && $isdaterange)
                    $where .= " and m.xtxnnum=c.imretailsl and m.xdate between '".$startdate."' and '".$enddate."' and ".$col." like '%".$text."%' and c.zemail = '".Session::get('suser')."'";
   ...
   function loadsupplier(){
    ...
    try{
        ...
        echo json_encode(array('message'=>$this->model->suppliers(),'result'=>'success'));
    }catch(Exception $e){
        $res = unserialize($e->getMessage()); 
   ...
   function loadoutlet(){
        ...
        try{
            ...
            if($supplier==""){
                throw new Exception(serialize(array('field'=>'Token', 'response'=>' Please enter supplier!')));
            }

            echo json_encode(array('message'=>$this->model->outletlist($supplier),'result'=>'success'));
        }catch(Exception $e){
            $res = unserialize($e->getMessage()); 
   ...
   function printinvoice($invoice){        
    ...
    $this->view->invdata = $this->model->getinvoiceforprint($invoice);
    $this->view->render("rawtemplate","abr/invoicecorp_view");
}
    ...
    function script(){
        ...
        <script>
            ...
            $('#clearcart').on('click', function(){
                localStorage.removeItem('corporatecart')
                itemcartdt = [];
                carttotal = 0;
                totalbv = 0;
                loadintable()
            ...
            if(localStorage.getItem('corporatecart') != null){
                itemcartdt = JSON.parse(localStorage.getItem('corporatecart'))
            }

            function loadsupplier(){
                $.ajax({
                        
                    url:\"".URL."corporateorder/loadsupplier\", 
                    type : \"POST\", 
                    //dataType: \"json\",                                 				
                    data : {token: $('#token').val()}, // post data || get data                    
                    beforeSend:function(){	
                        $('#company').html('<option default>Select Company</option>')
                        loaderon();   
                    },
                    success : function(result) {
                        ...
                        if(resultobj['result']=='success'){
                            $.each(resultobj['message'], function(key, val){
                                $('#company').append('<option value=\"'+val['xsup']+'\">'+val['xorg']+'</option>');
                            })
                        }
                        loaderoff();
            ...
            $('#company').on('change',function(){
                ...
                $.ajax({
                    ...
                    url:\"".URL."corporateorder/loadoutlet\", 
                    type : \"POST\", 
                    //dataType: \"json\",                                 				
                    data : {supplier: _this,token: $('#token').val()}, // post data || get data                    
                    beforeSend:function(){	
                        $('#outlet').html('<option default>Select Outlet</option><option default>Head Office</option>')
                        loaderon();     
                    },
                    success : function(result) {
                        //console.log(result)
                        ...
                        if(resultobj['result']=='success'){
                            $.each(resultobj['message'], function(key, val){
                                $('#outlet').append('<option value=\"'+val['outlet']+'\">'+val['outlet']+'</option>');
                            })
                        }
                        loaderoff();
            ...
            function loadinvoice(){
                $.ajax({
                    ...
                    url:\"".URL."corporateorder/loadinvoicedata\", 
                    type : \"GET\", 
                    dataType: \"json\",                                 				
            ...
            function loadintable(){
                ...
                var corporatecart = JSON.parse(localStorage.getItem('corporatecart'));
                var today = new Date();
                ...
                carttotal = 0;
                totalbv = 0;
                if(corporatecart!=null){
                    if(corporatecart.length!=0){
                        
                        $.each(corporatecart, function(key, val){
		 ...
		 'Thakurgaon', 'Bandarban',	'Brahmanbaria',	'Chandpur',	'Chittagong','Comilla','Cumilla','Cox’s Bazar',	
        ...
        $('#district').on('change', function(){
            ...
            $.each(selected, function(key, val){
                ...
                var districts = ['Barishal','Bhola','Chattogram','Chittagong','Khagrachari','Brahmanbaria','Chandpur',
                'Comilla','Cumilla','Cox&apos;s Bazar','Feni','Lakshmipur','Noakhali','Rangamati', 'Dhaka', 'Faridpur',
		 ...
		 $.each(districts,function(key,val){
			 $('#odcdistrict').append('<option vslue=\"'+val+'\">'+val+'</option>');			
        })
        ...
        $('#odcdistrict').on('change', function(){
            ...
            $.ajax({
                ...
                beforeSend:function(){	
                    loaderon();
                    $('#odc').html('<option default>Select ODC</option>');	
                },
                success : function(result) {
                    //console.log(result)
                    const resultobj = JSON.parse(result);
                ...
                $('#checkout').on('click', function(){
                    ...
                    $.ajax({
                        ...
                        url:\"".PAYGATE_URL."\", 
                        type : \"POST\",                                  				
                        data : {customer: $('#cin').html(),refrin:$('#refrin').val(),company:$('#company').val(),outlet:$('#outlet').val(),doc:'9',doctype:'Corporate Order',cusname:$('#delname').val(),odcid:$('#odc').val(),add1:$('#deladd').val(),add2:$('#deladd').val(),city:$('#thana').val(),district: $('#district').val(),postal:$('#postal').val(),callbackurl:\"".URL."corporateorder\",mobile:$('#mobile').val(),customertype:\"Customer\",apikey:\"36cfce7372fc99723569236e883dc4db39669cdf116a57d6d126e05fdea7be3c\",itemdt:itemcartdt}, 
                        ...
                        $('#cartmessage').html('<strong>Please add items in your cart!</strong>')
                    }
                }else{
                    $('#cartdiv').removeClass('bg-info');
                    $('#cartdiv').addClass('bg-danger');
            ...
            $('#btnadd').on('click', function(){
                ...
                $.ajax({
                    ...
                    url:\"".URL."corporateorder/showcustomer\", 
                    type : \"POST\",                                  				
            ...
            $('#searchcus').on('click', function(){
                ...
                $.ajax({
                    ...
                    url:\"".URL."corporateorder/searchcustomer\", 
                    type : \"POST\",                                  				
                   ...
                   $('#btnitemsearch').on('click', function(){
                    ...
                    $.ajax({
                        ...
                        url:\"".URL."corporateorder/getitems\", 
                        type : \"POST\",                                  				
                        data : {company:$('#company').val(), token: $('#token').val()}, 
                        beforeSend:function(){	
                            loaderon();   
                        },
                        success : function(result) {
                ...
                $('#searchitem').on('keyup',function(){
                    ...
                    const filterItems = (arr, query) => {
                     ...
                     $.each(filterItems(filterresult['message'], $(this).val()), function(key,val){
                        ...
                        $('#tblitemsearch tbody').append(`<tr><td><img src=\"".URL."public/images/productimage/`+val['itemcode']+`.jpg\" onerror=\"this.src='noimages.png';\" height=\"60\" width=\"60\"></td><td class=\"align-middle\">`+val['itemcode']+`</td><td class=\"align-middle\"><span class=\"itemdesc\">`+ val['itemdesc']+`</span> <br> Rate: <span class=\"text-success rate\">`+val['mrp']+` </span><br>BV : <span class=\"text-danger bv\">`+val['bv']+`<span></td><td class=\"align-middle text-center\"><input type=\"text\" value=\"1\" class=\"form-control form-control-sm\"></td><td class=\"align-middle\"><a id=\"`+val['itemcode']+`\" href=\"javascript:void(0)\" onclick=\"savecart($(this))\"><i class=\"fa fa-cart-plus\"></i></a></td></tr>`);
                     })
                  });
                  ...
                  $('#refrin').on('blur', function(){
                    ...
                    $.ajax({
                        ...
                        url:\"".URL."corporateorder/getrefrin/\"+refrin, 
                        type : \"GET\",                                  				
                        datatype:'json',
                        beforeSend:function(){	
                            loaderon();   
                  ...
                  $('#searchtxn').on('click', function(){
                    ...
                    $.ajax({
                        ...
                        url:\"".URL."corporateorder/searchtxn\", 
                        type : \"POST\",                                  				
                        data : dataobj,
                        beforeSend:function(){	
                            loaderon();   
                        ...
                        success : function(result) {
                            ...
                            if(resultobj['result']=='success'){
                                $.each(resultobj['message'], function(key, val){
                                    $('#txntable tbody').append(`<tr><td>`+val['txndate']+`</td><td>`+val['txnnum']+`</td><td>`+val['rin']+`</td><td>`+val['cusname']+`</td><td>`+val['mobile']+`</td><td><a href=\"javascript:void(0)\" onclick=\"showinvoiceitem(\``+val['txnnum']+`\`)\">View Detail</a></td><td><a target=\"_blank\" href=\"".URL."corporateorder/printinvoice/`+val['txnnum']+`\">Print</a></td></tr>`);
            ...
            function savecart(_this){
                ...
                localStorage.setItem('corporatecart',JSON.stringify(itemcartdt));
                loadintable();
            ...
            function showinvoiceitem(vinvoice){
                ...
                $.ajax({
                    ...
                    url:\"".URL."corporateorder/showtxndetail\", 
                    type : \"POST\",                                  				
