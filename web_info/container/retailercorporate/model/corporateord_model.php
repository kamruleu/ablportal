...
class Corporateord_model extends Model{
    ...
    function suppliers(){
        return $this->db->select('sesup',array("xsup,xorg")," xsuptype = 'Corporate' and zactive = 1 order by xorg");
    }

    function outletlist($supplier){
        $conditions[]= "xsup = ?";
        $params[]= $supplier;
		return $this->db->dbselectbyparam('sesupoutlet','xoutlet as outlet',$conditions,$params);
    ...
    function txnsearch($where){
        ...
        return $this->db->select('imretaildet m, imretail c',array("DATE_FORMAT(m.xdate,'%d-%m-%Y') as txndate,m.xtxnnum as txnnum,m.xrdin as rin,m.xdelname as cusname, m.xmobile as mobile"),$where);
    ...
    function supplierlist(){
        return $this->db->select('sesup',array("concat(xsup,' * ',xorg) sup"), "zactive=1");
    }
    ...
    function getinvoicedetail($invoice){
        ...
        $salesdetail = $this->db->dbselectbyparam('imretaildet m',"xtxnnum as txnnum,xitemcode as itemcode,xqty as qty,xrate as rate,xqty*xrate as total,(select xdesc from seitem c where m.xitemcode=c.xitemcode) as itemdesc, xqty*xbv as bv,(select xdesc from seitem c where m.xitemcode=c.xitemcode) as itemdesc, xqty*xbv as bv,xdeladd as addr, xcompany as company,xdelname as delname,xdeladd as deladd, xmobile as mobile",$conditionsdt,$params);
		return array("invoiceno"=>$invoice,"header"=>$salesheader,"detail"=>$salesdetail);

    }
    
    function getinvoiceforprint($invoice){
        $conditions[]= "imretailsl = ?";
        $conditions[]= "zemail = ?";
        ...
        $salesheader = $this->db->dbselectbyparam('imretail m',"imretailsl as txnnum,DATE_FORMAT(xdate,'%d-%m-%Y') as entrydate,xrdin as cin,xdelname as cusname, xstatus as status",$conditions,$params);
        $salesdetail = $this->db->dbselectbyparam('imretaildet m',"xtxnnum as txnnum,xitemcode as itemcode,xqty as qty,xrate as rate,xqty*xrate as total,(select xdesc from seitem c where m.xitemcode=c.xitemcode) as itemdesc, xqty*xbv as bv,(select xdesc from seitem c where m.xitemcode=c.xitemcode) as itemdesc, xqty*xbv as bv,xdeladd as addr, xcompany as company,xdelname as delname,xdeladd as deladd, xmobile as mobile",$conditionsdt,$params);
		return array("invoiceno"=>$invoice,"header"=>$salesheader,"detail"=>$salesdetail);
    ...
    function ablitemlist($text){
        ...
        $params[]= $text;
        $params[]= 1;
        return $this->db->dbselectbyparam("seitem","xitemcode as itemcode,xdesc as itemdesc,xmrp as mrp,xdp as bv,xstdcost as cost,xstdprice as myprice",$conditions,$params);
    }
