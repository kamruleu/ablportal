...
class Retailercart extends Controller{
    ...
    function init(){
        ...
        $this->view->render("templateadmin","abr/retailercart_view");
    }
   ...
   try{
       ...
       echo json_encode(array('message'=>$this->model->odclist($district),'result'=>'success'));
   }catch(Exception $e){
       $res = unserialize($e->getMessage()); 
    ...
    function script(){
        ...
        <script>
            ...
            $('#clearcart').on('click', function(){
                localStorage.removeItem('sesretailcart')
                itemcartdt = [];
                totalbv = 0;
                carttotal = 0;
                loadintable()
            ...
            if(localStorage.getItem('sesretailcart') != null){
                itemcartdt = JSON.parse(localStorage.getItem('sesretailcart'))
            }
            ...
            function loadintable(){
                ...
                var sesretailcart = JSON.parse(localStorage.getItem('sesretailcart'));
                var today = new Date();
                ...
                carttotal = 0;
                totalbv = 0;
                if(sesretailcart!=null){
                    if(sesretailcart.length!=0){
                        
                        $.each(sesretailcart, function(key, val){
		 ...
		 'Manikganj','Munshiganj','Mymensingh','Narayanganj','Narshingdi','Netrakona','Rajbari','Shariatpur','Chattogram',	
        ...
        $('#district').on('change', function(){
            ...
            $.each(selected, function(key, val){
                ...
                var districts = ['Barishal','Bhola','Chittagong','Chattogram','Khagrachari','Brahmanbaria','Chandpur',
        ...
        $('#odcdistrict').on('change', function(){
            ...
            $.ajax({
                ...
                $('#checkout').on('click', function(){
                    ...
                    $.ajax({
                        ...
                        url:\"".PAYGATE_URL."\", 
                        type : \"POST\",                                  				
                        data : {customer:\"".Session::get('suser')."\",refrin:'',company:'',outlet:'',doc:'0',doctype:'Retail Order',odcid:$('#odc').val(),cusname:$('#delname').val(),add1:$('#deladd').val(),add2:$('#deladd').val(),city:$('#thana').val(),district: $('#district').val(),postal:$('#postal').val(),callbackurl:\"".URL."buy\",mobile:$('#mobile').val(),customertype:\"Retailer\",apikey:\"36cfce7372fc99723569236e883dc4db39669cdf116a57d6d126e05fdea7be3c\",itemdt:itemcartdt}, 
            ...
            function savecart(_this){
                ...
                localStorage.setItem('sesretailcart',JSON.stringify(itemcartdt));
                loadintable();
