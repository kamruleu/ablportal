...
class Odcapply extends Controller{
	...
	function init(){
		...
		$this->view->render("templateadmin","abr/odcapply_view");
	...
	function loadarc(){
        ...
        try{
            ...
            echo json_encode(array('message'=>$this->model->loadarc(),'result'=>'success'));
        }catch(Exception $e){
	...
	function createretailer(){
		...
		$arccodearr = explode('|', filter_var($_POST['arccode'], FILTER_SANITIZE_STRING));
		$arccode = str_replace(" ","",$arccodearr[0]);
		...
		try{
			...
			$arcdt = $this->model->getarcdt($arccode);
			
			
			if(count($rindt)==0){
				throw new Exception(serialize(array('result'=>'error','field'=>'RIN', 'response'=>' not found!')));
			}
			...
			$dist = substr($data['xdistrict'], 0, 3);
			$than = substr($data['xthana'], 0, 3);

			$odcnum = $this->model->getodcnum($dist."-".$than);

			$rin = $rindt[0]['cus']; 
			
			$data['bizid']=100;
			$data['xdivision']=$arcdt[0]['xdivision'];			
			$data['xrdin']=$rin;			
			$data['odcnum']=$odcnum;	
			$data['offcode']=$arccode;
			$data['amount']=10000;
			$data['trxid']=$txnno;
			$data['xdate']=$date;
			$data['status']='Live';	
			...
			if($success>0){
				...
				$sendsms->send_single_sms('আমারবাজার এর বানিজ্যিক ভূবনে আপনাকে স্বাগতম। আপনার ODC আবেদন গৃহীত হয়েছে। আপনার ODC নম্বর '.$odcnum, $rindt[0]['mobile']);

				echo json_encode(array('result'=>'success','message'=>$odcnum.' created successfully!'));
			}
			else{	
	...
	function script(){
				...
				success : function(result) {
					...
					if(result['result']=='success'){
						$.each(result['message'],function(key,val){
							$('#arccode').append('<option>'+val.arccode+' | '+val.rep+'</option>');			
					   })
					}
		...
		$(document).ready(function(e){
			...
			$('#retailerform').on('submit',(function(e) {
				...
				$.ajax({
					type:'POST',
					url:\"".URL."odcapply/createretailer\", 
					data:formData,
					cache:false,
					contentType: false,
					processData: false,
		 ...
		 'Thakurgaon', 'Bandarban',	'Brahmanbaria',	'Chandpur',	'Chittagong','Chattogram','Cumilla','Cox’s Bazar',	
		 ...
		 var thana = [
			...
			{District:'CUMILLA', upazilla:'BARURA'},
			{District:'CUMILLA', upazilla:'BRAHMANPARA'},
			{District:'CUMILLA', upazilla:'BURICHONG'},
			{District:'CUMILLA', upazilla:'CHANDINA'},
			{District:'CUMILLA', upazilla:'CHOUDDAGRAM'},
			{District:'CUMILLA', upazilla:'CUMILLA-S'},
			{District:'CUMILLA', upazilla:'CUMILLA-S DAKSIN'},
			{District:'CUMILLA', upazilla:'DAUDKANDI'},
			{District:'CUMILLA', upazilla:'DEBIDWAR'},
			{District:'CUMILLA', upazilla:'HOMNA'},
			{District:'CUMILLA', upazilla:'LAKSHAM'},
			{District:'CUMILLA', upazilla:'LALMAI'},
			{District:'CUMILLA', upazilla:'MEGHNA'},
			{District:'CUMILLA', upazilla:'MONOHORGANJ'},
			{District:'CUMILLA', upazilla:'MURADNAGAR'},
			{District:'CUMILLA', upazilla:'NANGALKOT'},
			{District:'CUMILLA', upazilla:'TITAS'},
			{District:'CUMILLA', upazilla:'CHHAGALNIYA'},
			{District:'CUMILLA', upazilla:'DAGANBHUIYAN'},
